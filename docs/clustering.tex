...
\textbf{Sick nodes don't stand for election.} Before taking the lease a node \ripmono{ffprobe}s its own channel URL — proving ErsatzTV is emitting decodable video, not merely answering HTTP. A node whose ErsatzTV is wedged stands down and lets a standby take it, rather than winning the election and airing silence. Standbys probe every tick; the leader probes every \ripmono{Z0\_HEALTH\_EVERY} (30 s) while healthy and every tick once a probe fails, because each probe is a channel session ErsatzTV logs, and probing every 5 s buried the real log lines.
