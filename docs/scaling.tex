...
\begin{ripbullets}
\item \ripmono{/api/status} is polled every 30 s per viewer: 1,000 viewers is 33 req/s, 10,000 is 333 req/s on a one-core box. Split hostnames: an ingest host (DNS-only, RTMP) and the watch host (proxied, JSON only). Caddy stamps \ripmono{Cache-Control: public, s-maxage=10} on \ripmono{/api/status}, but JSON is not cacheable by default, so add a Cache Rule making that one path eligible (respect origin TTL); then the tower sees one request per 10 s per cache tier. The zone's browser check refuses the stock \ripmono{Python-urllib} agent on every proxied host, so anything scripted against the watch host must send a named agent; the now-playing bridge does. The push must already be on the ingest host before the flip: an RTMP session on a proxied name dies with it. Cloudflare's terms require a paid product (R2 counts) for video through the CDN; the master playlist is a few hundred bytes and the segments no longer route through the watch host, so nothing more is needed unless something starts asking the tower for \ripmono{/hls/0/*}.
